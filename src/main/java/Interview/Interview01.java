...
public: bu yönteme kimin erişebileceğini belirtmek için kullanılan bir erişim değiştiricidir
ve bu yöntemin herhangi bir sınıf tarafından erişilebilir olacağı anlamına gelir.
static: Java'da sınıf tabanlı olduğunu tanımlayan bir anahtar kelimedir.
main (), bir sınıfın oluşumunu oluşturmadan erişilebilmesi için Java’da static yapılır.
...
C8: ArrayList hızlı olması için senkronize edilmemiştir ancak Vector,
iş parçacığı açısından güvenli olduğundan yavaş olması için senkronize edilmiştir.
...
C10: Heap hafızası bir Application in tüm parçaları tarafından kullanılır.
Objectler Heap içinde depolanır ve herkesçe genel kullanıma müsaittir.
Stack hafıza ise sadece bir yürütme dizisi tarafından kullanılır ve diğerleri tarafından asla kullanılamaz.
...
(Uyarı: Protected a dikkat edin lütfen yani ayni package ya da farklı package olması önemli değil
sadece farklı package in alt class in da olmayan dosyadan erişim sağlanamıyor.)
...
C19: Bu bir programlama modeli veya yaklaşımı, büyük ve karmaşık kodlu programlar için idealdir
ve aktif olarak güncellenmesi ve sürdürülmesi gerekir.
Programlar, mantık ve işlevler yerine objeler etrafında düzenlenmiştir.
...
C25: Final keywordunu bir variable ile kullandığımızda onun değeri bir kez atandığı için değiştirilemez.
...
C28: This() aynı class in varsayılan Constructor’larını çağırmak için kullanılır,
ancak super() onları ebeveyn / temel class tan çağırmak için kullanılır.
...
C29: Heap hafıza içinde String’lerin toplandığı havuz yani yerdir.
Bir Object oluşturulduğu zaman onun bu havuzda önceden olup olmadığını kontrol eder.
...
C35: Basitçe, String objesi oluşturulduktan sonra durumunun değiştirilemeyeceği anlamına gelir.
Uygulamanın güvenlik önbelleğini, senkronizasyonunu ve performansını artırır.
...
C38: Collection bir mimar gibi bir grup nesne objeyi depolamak ve işlemek için hareket eden bir çerçeveden
framework tan oluşur.
...
C40: Bu soruya en iyi örnek; kamera, hesap makinesi, mp3 çalar yada uzaktan kumanda gibi
kullanılan bir cep telefonudur.
...
C43: Abstraction method ve statik sabitlerin birleşimidir. Herhangi bir Constructor’a sahip değildir.
...
Aynı Class’ta ki methodlar aynı adı paylaşır ancak her methodun farklı sayıda parametresi olmalıdır.
Veya farklı tür ve sıraya sahip parametreler.
...
C51: Hayır yapmazsınız, önce class içinde inheritance yapmanız lazım. Buda imkânsız özel
 ve static olduğu için method.
...
methodun çalıştırılacağını, aynı method adı var ise farklı ebeveyn sınıflarında.
(52. soruda sunu kaçırmayalım burada ki multilevel demek bir den fazla parent class olayı yoksa Java hybrid ve
hierarchical gibi çoklu Inheritance lari destekliyor) (Derste burada iki babalı ya da iki anneli bi çocuk olmaz
demişti Süleyman Bey. C++ da bunu destekliyor ama Java’da desteklemiyor)
...
A58: Bu açık olarak bir objenin kopyasının yapılması prosedürüne denir.
...
İstisna (exception): Hatalı girdi nedeniyle ortaya çıkan durumdur veya insan hatası vb.
Çoğu durumda kurtarmak mümkündür.
...
C66: Kendi Exception’ınızı oluşturmak için exception class a extends edin veya onun
alt klaslarından herhangi birinden extends edin.
...
C74: İş parçacığı, programlanmış talimatların en küçük parçasıdır. Bir programlayıcı
tarafından bağımsız olarak yürütülebilir.
...
Runnable ara yüzünü uygulayarak. Thread’i genişleterek.


* S76: Garbage collection nedir?
C76: Java'da çöp toplama, örtük implicit olarak hafıza yönetimine yardımcı olan bir programdır.
Java'da, yeni anahtar sözcüğü kullanarak dinamik olarak objeler oluşturabilirsiniz,
herhangi bir kez oluşturulduktan sonra biraz hafıza tüketir.
